...
public class StylesCollectionConditions<TConditions> : Conditions<TConditions>
{
    ...
    public StringStyleConditions<TConditions> this[string styleName]
    {
        get
        {
            return Share(new StringStyleConditions<TConditions>(_conditions, _elementHandler, styleName, _timeout, _pollingInterval, $"{styleName} style of the {_elementHandler.ComponentMetadata.Name}", _logger));
        }
    }
    ...
    public NumericStyleConditions<TConditions, double> Opacity =>
        Share(new NumericStyleConditions<TConditions, double>(_conditions, _elementHandler, "opacity", _timeout, _pollingInterval, $"opacity of the {_elementHandler.ComponentMetadata.Name}", _logger));
}
