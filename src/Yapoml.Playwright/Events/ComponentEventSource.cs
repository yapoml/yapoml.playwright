...
namespace Yapoml.Playwright.Events
{
    public class ComponentEventSource : IComponentEventSource
    {
        ...
        public void RaiseOnFoundComponent(string by, IPage context, ILocator webElement, ComponentMetadata componentMetadata)
        {
            OnFoundComponent?.Invoke(this, new FoundElementEventArgs(by, context, webElement, componentMetadata));
        }

        public void RaiseOnFoundComponents(string by, IPage context, IReadOnlyList<ILocator> elements, ComponentsListMetadata componentsListMetadata)
        {
            OnFoundComponents?.Invoke(this, new FoundElementsEventArgs(by, context, elements, componentsListMetadata));
        }
    }
}
